...
class Flatliner:
    def __init__(self):
        ...
        # for testing to make sure all handlers are hit
        self.hit = {k: 0 for k in self.node_handlers}

    def all_hit(self) -> bool:
        """
        returns if all handlers were tested at least once and prints
        out the handlers that were not tested.
        """
        for k, v in self.hit.items():
            if not v:
                print(f'not tested: {k}')
        return 0 not in self.hit.values()
    ...
    def apply_handler(self, node, cont=None):
        node_type = type(node)
        node_repr = self.node_handlers.get(node_type, self.handle_error)(node, cont)
        if node_type in self.hit:
            self.hit[node_type] += 1
        return node_repr
